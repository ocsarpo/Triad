(function (root, factory) {
  ...
  const CHROME = {
    ...
    '각 AI가 독립적으로 응답 · #검토 교차 검토 · #대화 직접 대화': 'Each AI answers independently · #검토 cross-review · #대화 direct AI talk',
  ...
  const MSG = {
    ...
    welcomeText: { en: 'In independent mode, send without tags and both AIs respond. In agent collaboration, the lead AI asks the other AI when needed and continues with the answer. Add #검토 for a cross-review of the finished answer, or #대화 to watch the two AIs talk it out directly. Reference project files with @document.', ko: '독립 실행에서는 태그 없이 보내면 두 AI가 응답합니다. 에이전트 협업에서는 시작 AI가 필요할 때 상대 AI에게 질문하고 답을 돌려받아 작업을 계속합니다. 메시지에 #검토를 붙이면 완성된 답변을 상대 AI가 교차 검토하고, #대화를 붙이면 두 AI가 직접 대화하는 모습을 볼 수 있습니다. @문서로 프로젝트 파일을 참조할 수 있습니다.' },
