...
test('대기열 생명주기 변경의 앱 버전은 이 테스트에서 검증한다', () => {
  assert.match(plist, /<key>CFBundleShortVersionString<\/key>\s*<string>0\.48\.10<\/string>/);
  assert.match(plist, /<key>CFBundleVersion<\/key>\s*<string>80<\/string>/);
});
...
test('독립 실행은 대상 슬롯과 공유 문서 충돌을 분리해 판단한다', () => {
  ...
  assert.match(renderer, /enqueueIndependentBatch\(routed\.targets,routed\.prompts,original,userMessageId,\{objective:references\.clean,sharedContext:null,documentId:requestedDocumentId,recentContexts,referencePacket\}\)/);
