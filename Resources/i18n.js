(function (root, factory) {
  ...
  const CHROME = {
    ...
    '각 AI가 독립적으로 응답 · 필요하면 서로 호출 · #검토 교차 검토 · #대화 직접 대화': 'Each AI answers independently · they call each other when needed · #검토 cross-review · #대화 direct AI talk',
    '교차 검토 대기열 등록': 'Cross-review queued',
    '자동 (폴더 매칭)': 'Auto (folder match)',
    '최근': 'Recent',
    '최근 사용한 작업 폴더': 'Recently used workspaces',
    '이 AI의 작업 폴더에서 터미널 열기': "Open a terminal in this AI's workspace",
    '⌨ 터미널': '⌨ Terminal',
    '상호 대화 시작': 'AI conversation started',
    '상호 대화 턴': 'AI conversation turn',
    '상호 대화 완료': 'AI conversation complete',
    // composer — pins / tags
    ...
    '아직 복사할 원문이 없습니다.': 'There is nothing to copy yet.',
    '원문을 클립보드에 복사하고 있습니다.': 'Copying the original to the clipboard.',
    '나': 'Me',
    '회의실': 'Room',
    '방금': 'just now',
    '확인 불가': 'Unknown',
    // run-log (trace) titles
    'AI 간 호출 도구 준비 실패': 'AI-to-AI call tool setup failed',
    'Codex 오류': 'Codex error',
    'MCP 협업 완료 후 중복 인계 마커 무시': 'Ignored duplicate handoff marker after MCP collaboration',
    '계정 인증 실패': 'Account authentication failed',
    '계정 잔여량 갱신': 'Account usage updated',
    '공유 문서 기여 미확인': 'Shared-doc contribution unconfirmed',
    '공유 문서 목록 오류': 'Shared-doc list error',
    '공유 문서 생성': 'Shared doc created',
    '공유 보드 갱신': 'Shared board updated',
    '다음 작업 새 세션': 'New session for next task',
    '대기 독립 실행 시작': 'Queued independent run started',
    '대기 작업 시작': 'Queued task started',
    '대기 작업 취소': 'Queued task canceled',
    '대기 협업 시작': 'Queued collaboration started',
    '독립 실행 묶음 대기열 등록': 'Independent run batch queued',
    '마무리 단계에서 추가 위임 요청': 'Extra handoff requested during wrap-up',
    '명령 실행': 'Command executed',
    '명령 전달': 'Command dispatched',
    '보조 AI 호출 실패': 'Assisting-AI call failed',
    '비정형 출력': 'Unstructured output',
    '빈 응답 진단': 'Empty-response diagnostics',
    '사용량 조회 실패': 'Usage lookup failed',
    '사용량 한도 상태': 'Rate-limit status',
    '사용자 중지': 'Stopped by user',
    '세션 시작': 'Session started',
    '순차 대기열 등록': 'Queued sequentially',
    '실행 실패': 'Run failed',
    '에이전트 협업 시작': 'Agent collaboration started',
    '에이전트 협업 완료': 'Agent collaboration complete',
    '응답 메시지 완료': 'Response message complete',
    '응답 생성 완료': 'Response generation complete',
    '응답 스트림 완료': 'Response stream complete',
    '이번 요청 토큰 사용량': 'Token usage for this request',
    '작업 완료': 'Task complete',
    '작업 처리 시작': 'Task processing started',
    '저장된 세션을 찾지 못해 새 세션으로 자동 재시도': 'Saved session not found; auto-retrying with a new session',
    '문맥 한도 초과, 새 세션으로 자동 재시도': 'Context limit hit; auto-retrying with a new session',
    '전송 준비 실패': 'Send prep failed',
    '최대 위임 횟수 도달': 'Max handoffs reached',
    '추론 단계 시작 · 내용 비공개': 'Reasoning step started · hidden',
    '표준 오류 출력': 'Standard error output',
    '협업 순차 대기열 등록': 'Collaboration queued sequentially',
    '협업 인계 형식 오류': 'Collaboration handoff format error',
    '협업 흐름 중단': 'Collaboration flow stopped',
    'Claude 실행 오류': 'Claude run error',
    '프로세스 정상 종료': 'Process exited normally',
    'CLI 계정 연결 확인': 'CLI account connection verified',
    'CLI 계정 연결 필요': 'CLI account connection needed',
    'CLI 계정 로그아웃': 'CLI account logged out',
    'CLI 계정 연결 완료': 'CLI account connected',
    // trace detail / alert sentences
    '협업': 'Collaboration',
    '현재 실행만 중지하고 대기 작업은 유지합니다.': 'Stops only the current run; queued tasks are kept.',
    '사용자가 세션을 초기화했습니다.': 'The user reset the session.',
    '공유 문서를 불러오지 못했습니다.': 'Could not load shared documents.',
    '응답 없음': 'No response',
    '인증 작업 실패': 'Authentication task failed',
    '계정 인증에 실패했습니다.': 'Account authentication failed.',
    // settings panel
    ...
    '계속 유지 (기본)': 'Keep session (default)',
    'CLI가 스스로 문맥을 관리하므로 세션은 계속 이어집니다. 아래 요약의 토큰 수치는 참고용입니다.': 'The CLI manages its own context, so the session keeps going. The token figures below are informational.',
    '계속 유지': 'Keep session',
  ...
  const MSG = {
    ...
    autoLeadPicked: { en: ' (auto-picked: {reason})', ko: ' (자동 선택: {reason})' },
    dialogStart: { en: '{lead} opens a conversation with the other AI — their exchange appears right here in the chat.', ko: '{lead}가 상대 AI와 대화를 시작합니다 — 두 AI가 주고받는 말이 이 채팅에 그대로 표시됩니다.' },
    dialogDone: { en: 'The AI-to-AI conversation has ended.', ko: '두 AI의 대화가 끝났습니다.' },
    flowDialogState: { en: '{agent} · talking ({n}/{max})', ko: '{agent} · 대화 중 ({n}/{max})' },
    ...
    handoffAsk: { en: '**→ {to} · question · {count}**\n\n{question}{reason}', ko: '**→ {to} · 질문 · {count}**\n\n{question}{reason}' },
    callReason: { en: '\n\nreason: {r}', ko: '\n\n호출 이유: {r}' },
    ...
    handoffAnswer: { en: '**→ {from} · answer ✓ · {count}**\n\n{body}', ko: '**→ {from} · 답변 ✓ · {count}**\n\n{body}' },
    ...
    msgStarted: { en: 'Started {time}', ko: '답변 시작 {time}' },
    msgCompleted: { en: 'Completed {time}', ko: '작성 완료 {time}' },
    msgWriting: { en: 'Writing…', ko: '작성 중…' },
    msgThinking: { en: 'Thinking…', ko: '생각 중…' },
    accountOf: { en: '{name} account', ko: '{name} 계정' },
    acctChecking: { en: 'Checking connection…', ko: '연결 확인 중…' },
    acctNeeded: { en: 'Not connected', ko: '연결 필요' },
    acctCodexApi: { en: '✓ API account connected', ko: '✓ API 계정 연결됨' },
    acctCodexChatGpt: { en: '✓ ChatGPT connected', ko: '✓ ChatGPT 연결됨' },
    acctCodex: { en: '✓ Codex account connected', ko: '✓ Codex 계정 연결됨' },
    acctClaudeMethod: { en: '✓ Claude {method} connected', ko: '✓ Claude {method} 연결됨' },
    acctClaude: { en: '✓ Claude account connected', ko: '✓ Claude 계정 연결됨' },
    handoffReturn: { en: '{from} → {to} · delivering the answer and resuming the original task.', ko: '{from} → {to} · 답변을 전달하고 원래 작업을 재개합니다.' },
    brokerToolFail: { en: 'Could not prepare the AI-to-AI call tool.', ko: 'AI 간 호출 도구를 준비하지 못했습니다.' },
    welcomeText: { en: 'One room: send without tags and both AIs answer; #a/#b picks one. Any run can call the other AI (ask_agent) when needed, and can hand its result over for a cross-review on its own. Add #검토 to request a review, or #대화 to watch the two AIs talk directly. Reference project files with @document.', ko: '방은 하나입니다. 태그 없이 보내면 두 AI가 각자 답하고, #a/#b로 한 명을 지정합니다. 어떤 실행이든 필요하면 상대 AI를 호출(ask_agent)하고, 산출물은 스스로 상대에게 교차 검토를 맡기기도 합니다. #검토를 붙이면 답변을 상대가 검토하고, #대화를 붙이면 두 AI가 직접 대화합니다. @문서로 프로젝트 파일을 참조할 수 있습니다.' },
    usageLeft: { en: '{label}: {pct}% left', ko: '{label}: {pct}% 남음' },
    usageResets: { en: ' · resets {reset}', ko: ' · {reset} 초기화' },
    usageLastChecked: { en: 'Last checked: {time}', ko: '마지막 확인: {time}' },
    usageClaudeNote: { en: 'Claude shows the latest limit info received during a run.', ko: 'Claude는 작업 중 수신한 최신 한도 정보를 표시합니다.' },
    usageErr: { en: 'Error: {error}', ko: '오류: {error}' },
    usageNoData: { en: 'No account limit info was provided.', ko: '계정 한도 정보가 제공되지 않았습니다.' },
    // composed run-log (trace) titles
    traceReturnAux: { en: 'Returning the assist answer to {name}', ko: '{name}에게 보조 답변 반환' },
    traceDeliverQ: { en: 'Delivering the question to {name}', ko: '{name}에게 질문 전달' },
    traceReturnAns: { en: 'Returning the answer to {name}', ko: '{name}에게 답변 반환' },
    traceAuxCall: { en: 'Assist call to {name}', ko: '{name} 보조 호출' },
    traceStepStart: { en: 'Step start · {type}', ko: '단계 시작 · {type}' },
    traceStepDone: { en: 'Step done · {type}', ko: '단계 완료 · {type}' },
    traceToolDone: { en: 'Tool done · {tool}', ko: '도구 완료 · {tool}' },
    traceToolCallSrv: { en: 'Tool call · {server} {tool}', ko: '도구 호출 · {server} {tool}' },
    traceToolCall: { en: 'Tool call · {tool}', ko: '도구 호출 · {tool}' },
    traceCmdExit: { en: 'Command exited', ko: '명령 종료' },
    traceStatus: { en: 'Status · {status}', ko: '상태 · {status}' },
    traceSessionModel: { en: 'Session started · {model}', ko: '세션 시작 · {model}' },
    traceGenStart: { en: 'Response start · {model}', ko: '응답 생성 시작 · {model}' },
    traceProcExit: { en: 'Process exited · code {code}', ko: '프로세스 종료 · code {code}' },
    // collaboration phase + task labels (also shown in status line)
    taskAnswerTo: { en: '{name} answer', ko: '{name} 질문 응답' },
    taskResume: { en: 'Resuming after applying the answer', ko: '답변 반영 후 작업 재개' },
    taskStart: { en: 'Starting the work', ko: '작업 시작' },
